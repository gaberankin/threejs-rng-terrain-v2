...
function getCameraSpans():
  ...
  if (
    !(
      x instanceof HTMLSpanElement &&
      y instanceof HTMLSpanElement &&
      z instanceof HTMLSpanElement
    )
  ) {
    return null;
  }
...
function generateGeometry(): THREE.BufferGeometry {
...
function init(): void {
  ...
  function onGenerateGeometryClick(): void {
    const hills = generateGeometry();
    plane.geometry.dispose();
    plane.geometry = hills;
    return;
  }

  const generateGeometryButton = document.getElementById("generate-geometry");
  if (generateGeometryButton instanceof HTMLButtonElement) {
    generateGeometryButton.addEventListener("click", onGenerateGeometryClick);
  }
